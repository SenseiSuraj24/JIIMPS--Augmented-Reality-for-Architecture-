...
3. **Feature Detection:** 
   ...
   - **Rooms:** Uses connected component analysis to segment enclosed spaces.
   - **Text:** Uses Optical Character Recognition (OCR) to extract dimension text.
...
|---|---|---|
| `detectWalls.m` | Detects wall segments using Hough Transform | Detect relevant floor plan features |
| `detectRooms.m` | Segments rooms using connected components | Advanced: additional information |
| `extractDimensionText.m` | Extracts wall length labels via OCR | Advanced: auto measurement |
...
2. **Prepare Floor Plan:** Print your floor plan (or use `test_floorplan.png`) and tape an **AprilTag (tag36h11)** to a known location on the paper.
